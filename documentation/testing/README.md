...
| `vite.config.ts` → lint override for `**/*.spec.{ts,tsx}` | `no-magic-numbers` off, `max-lines-per-function` 200.                                                                                                          | A `__tests__/*.test.ts` file would NOT get these relaxations. Use `*.spec.ts(x)`.                                           |

## Conventions in one glance

- Test file: `foo.spec.tsx` next to `foo.tsx`. No `__tests__/` directories.
- Explicit imports from `vite-plus/test` when you need `vi`, `expectTypeOf` (lint rule `vite-plus/prefer-vite-plus-imports`).
- Run one file: `vp test src/path/to/foo.spec.tsx`. Run everything: `vp test`. Lint/format/types: `vp check`.
